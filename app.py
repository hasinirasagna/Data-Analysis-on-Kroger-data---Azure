...
config = {
    'host': 'bhavana1.mysql.database.azure.com',
    'user': 'rootbhavana',
...
def Search():
    ...
    if request.method == 'POST' and 'search' in request.form:
        ...
        if not re.match(r'\d+', number):
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT h.HSHD_NUM, t.BASKET_NUM, t.PURCHASE_, p.PRODUCT_NUM, p.DEPARTMENT, p.COMMODITY,t.SPEND, t.UNITS, t.STORE_R, t.WEEK_NUM, t.YEAR, h.L, h.AGE_RANGE,h.MARITAL,h.INCOME_RANGE, h.HOMEOWNER, h.HSHD_COMPOSITION, h.HH_SIZE, h.CHILDREN FROM households AS h JOIN transactions AS t ON h.HSHD_NUM = t.HSHD_NUM JOIN products AS p ON t.PRODUCT_NUM = p.PRODUCT_NUM where h.HSHD_NUM=%s",
                (number,))
            data = cur.fetchall()
            if data:
...
def upload():
    ...
    if request.method == 'POST':
        ...
        app.logger.info(hdata)
        tdata = request.files['transactions']
        pdata = request.files['products']
        conn = mysql.connector.connect(**config)
        cur = conn.cursor()
        if True:
            # hdata.filename == '' or tdata.filename == '' or pdata.filename == '' :
            ...
            for i, row in csvData.iterrows():
                if (pd.isna(row).any()):
                    continue  # skip row if any column value is null
                query += '{},'.format((row['HSHD_NUM'], row['L'], row['AGE_RANGE'], row['MARITAL'], row['INCOME_RANGE'],
            ...
            tdata.save(file_path)
            print("file path", file_path)
            col_names = ['BASKET_NUM', 'HSHD_NUM', 'PURCHASE_', 'PRODUCT_NUM', 'SPEND', 'UNITS', 'STORE_R', 'WEEK_NUM',
                         'YEAR']
            csvData = pd.read_csv(file_path, names=col_names, header=0, nrows=10000)
            query = 'INSERT INTO transactions (BASKET_NUM,PURCHASE_,SPEND,UNITS,STORE_R,WEEK_NUM,YEAR,HSHD_NUM,PRODUCT_NUM) VALUES'
            for i, row in csvData.iterrows():
                if (pd.isna(row).any()):
                    continue  # skip row if any column value is null
                query += '{},'.format((row['BASKET_NUM'], row['PURCHASE_'], row['SPEND'], row['UNITS'], row['STORE_R'],
                                       row['WEEK_NUM'], row['YEAR'], row['HSHD_NUM'], row['PRODUCT_NUM']))
            query = query[:len(query) - 1]
            cur.execute(query)
